...
def choose(paragraphs, select, k):
    ...
    "*** YOUR CODE HERE ***"
    for p in paragraphs:
        if select(p) == True: # Narrow k，get the next paragraph
            k -= 1
        if k == -1: # Find the k-th paragraph
            return p
    return ''
    # END PROBLEM 1
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def check_has_topic(paragraph):
        p_t = lower(remove_punctuation(paragraph)) # Remove punctuation and lower it.
        words = split(p_t) # Paragraph splits string list.
        for topic_word in topic:
            if lower(topic_word) in words:
                return True
        return False
    return check_has_topic
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    # Both empty
    if reference_len == 0 and typed_len == reference_len:
        return 100.0
    if typed_len == 0:
        return 0.0
    ...
    idx = 0
    match_count = 0
    # Comparison typed word and referenced word.
    while idx < min(typed_len, reference_len):
        if typed_words[idx] == reference_words[idx]:
            match_count += 1
        idx += 1
    return match_count / typed_len * 100.0
    # END PROBLEM 3
