...
def roll_dice(num_rolls, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    points = 0
    sad_flag = False # Sow Sad Flag
    for i in range(0, num_rolls):
        score = dice() 
        if score == 1: # Execute all dices
            sad_flag = True
        points += score
    if sad_flag:
        return 1
...
def piggy_points(score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    target = score * score # Square score
    min_digt = 9 # Set min digit
    # Special 0
    if target == 0: 
        min_digt = 0
    while target != 0:
        digit = target % 10 # Get every digits
        if digit < min_digt: # Update min digit
            min_digt = digit
        target //= 10
    return 3 + min_digt
    # END PROBLEM 2
