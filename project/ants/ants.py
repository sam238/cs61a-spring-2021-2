...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while current_place != beehive:
            # If find bees exist the current place
            if self.min_range <= i <= self.max_range and len(current_place.bees) > 0:
                return bee_selector(current_place.bees)
            # Find the front place
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= 0:
            amount += self.damage
        # Copy from bees
        current_bees = current_place.bees[:]
...
class TankAnt(ContainerAnt):
    """TankAnt provides protection to other Ants and offense to bees"""
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        if not self.is_queen:
            ...
            while next_place:
                if next_place.ant:
                    if not next_place.ant.is_doubled:
                        next_place.ant.damage *= 2
                        next_place.ant.is_doubled = True
                    if next_place.ant.is_container() and next_place.ant.contained_ant:
                        if not next_place.ant.contained_ant.is_doubled:
                            next_place.ant.contained_ant.damage *= 2
                            next_place.ant.contained_ant.is_doubled = True
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= 0 and self.is_queen:
            bees_win()
